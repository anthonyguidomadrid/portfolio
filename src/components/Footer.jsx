export const Footer = ({logo, footerItems, socialMedia}) => {
    return (
        <div className="bg-slate-900 text-white px-10 md:px-24">
            ...
            {footerItems?.length && footerItems.map(item => {
                return <a href={`/#${item.toLowerCase().replace('', '')}`} className="uppercase px-5 text-xs sm:text-sm md:text-base">{item}</a>
            })}
            </div>
            {socialMedia?.length && <div className="flex justify-center py-5">{socialMedia.map(elm => {
                    return <a href={elm.link} target="_blank" rel="noreferrer"><img src={elm.url} alt={elm.name} className="h-7 w-7 mx-2"></img></a>
                })}</div>}
