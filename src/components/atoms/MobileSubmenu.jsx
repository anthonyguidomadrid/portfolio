...
export const MobileSubMenu = ({projectsContent, locale, isSubMenuOpen, setMenuOpen, setIsSubMenuOpen}) => {
    ...
    return (
        <div className={classNames('text-xs text-slate-200/75 flex flex-col opacity-0 max-h-0 transition-all duration-1000', {'max-h-96 opacity-100' : isSubMenuOpen})}>
        {projectsContent.length > 0 && projectsContent.map((project, idx) => {
            ...
            return !isSubMenuOpen ? null : ( <HashLink
            key={idx}
