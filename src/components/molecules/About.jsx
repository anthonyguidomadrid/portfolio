...
export const About = ({aboutContent, locale}) => {
    ...
    <section className="md:flex" id={id}>
        ...
        <div className="md:w-1/2 bg-slate-900 p-10 flex justify-center text-white flex-col">
            ...
            <p className={'font-mono text-4xl text-gray-100 my-5 lg:pb-5'}>{subtitle[index]}</p>
            <div dangerouslySetInnerHTML={{ __html: description }} />
            {cv?.url && <div className='lg:py-10 transition-all duration-500'><Button text={cv?.text ?? 'Download CV'} link={cv?.url} newTab={true} additionalClasses={'uppercase mt-5'}/></div>}
        </div>
    </section>
    )
}